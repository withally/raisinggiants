...
export function StickyBlueprintBar({ hidden = false }: StickyBlueprintBarProps) {
  ...
  return (
    <aside
      className="fixed bottom-0 left-0 right-0 z-40 bg-stone-900/95 backdrop-blur-sm border-t border-stone-700"
    ...
    >
      <div className="max-w-3xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-baseline gap-1 sm:gap-3 text-center sm:text-left">
          <span
            className="text-stone-100 text-sm font-semibold"
            style={{ fontFamily: "var(--font-display)" }}
          >
            The Blueprint is coming.
          </span>
          <span className="text-stone-400 text-xs hidden sm:inline" aria-hidden="true">
            —
          </span>
        ...
        <Link
          href="/blueprint"
          className="shrink-0 inline-flex items-center justify-center rounded-full bg-amber-500 px-5 py-2 text-xs font-semibold text-stone-900 hover:bg-amber-400 transition-colors duration-200 min-h-[36px] min-w-[44px]"
        >
          Notify me
        </Link>
      </div>
